...
def fetch_questions(max_questions=None):
...
def fetch_questions_and_answers(max_questions=None, max_workers=100):
    fetched_questions = fetch_questions(max_questions)
    if fetched_questions:
        threads = []
        semaphore = Semaphore(max_workers) 

        def thread_target(question):
            with semaphore:  
                get_and_append_accepted_answer(STACK_EXCHANGE_API_KEY, question)

        for question in fetched_questions:
            thread = threading.Thread(target=thread_target, args=(question,))
